        ...
        public void Popup()
        {
            if (!disposed)
            {
                if (!frmPopup.Visible)
                {
                    ...
                    if (Scroll)
                    {
                        posStart = Screen.PrimaryScreen.WorkingArea.Bottom/2;
                        posStop = Screen.PrimaryScreen.WorkingArea.Bottom/2 - frmPopup.Height;
                    }
                    else
                    {
                        posStart = Screen.PrimaryScreen.WorkingArea.Bottom/2 - frmPopup.Height;
                        posStop = Screen.PrimaryScreen.WorkingArea.Bottom/2 - frmPopup.Height;
                    }
                    opacityStart = 0;
                    opacityStop = 1;
                    ...
                    frmPopup.Location = new Point(Screen.PrimaryScreen.WorkingArea.Right/2 - frmPopup.Size.Width/2 - 1, posStart);
                    frmPopup.Show();
                    isAppearing = true;
